...
def _keras_io_shapes(model: tf.keras.Model):
    in_shapes = [tuple(x.shape) for x in model.inputs] if hasattr(model, "inputs") else []
    out_shapes = [tuple(x.shape) for x in model.outputs] if hasattr(model, "outputs") else []
    n_classes = None
    if out_shapes:
        last = out_shapes[0]
...
def _analyze_keras(obj: KerasLike, details: bool) -> dict:
    ...
    info = {
        ...
        "parametros_totais": int(model.count_params()),
...
def _keras_bench(model: tf.keras.Model, runs=50, warmup=10) -> float:
    ...
    ishapes = [tuple(x.shape) for x in model.inputs]
    if not ishapes:
        return float("nan")
...
def _tflite_io_details(interpreter: tf.lite.Interpreter):
    def qinfo(d):
        # (scale, zero_point) e possíveis arrays per-channel
...
def _analyze_tflite(p: PathLike, details: bool) -> dict:
    ...
    # “neurônios” = soma de outputs de FULLY_CONNECTED
    dense_units = 0
    if op_hist:
        # precisa dos detalhes dos tensores para estimar units
    ...
    *,
    ...
    if isinstance(obj, tf.keras.Model):
        ...
        if ext in {".keras", ".h5"} or (path.is_dir() and (path / "saved_model.pb").exists()):
            ...
            model = tf.keras.models.load_model(str(path)) if benchmark else None
            bench_ms = _keras_bench(model, runs=runs, warmup=warmup) if benchmark else None
        elif ext == ".tflite":
